    ...
    if filename.endswith(('.jpg', '.jpeg', '.png', '.webp')):
        ...
        while True:
            ...
            if key == ord(' '):  # Нажатие пробела для сохранения
                ...
                print(f"Изображение сохращено в {ready_image_path}")
                break
            elif key == 27:  # Нажатие Esc для пропуска изображения
                print("Изображение пропущено")
                break
            elif key in [ord(str(i)) for i in range(1,10)]:  # Выбор класса
                current_class = int(chr(key))-1
                print(f"Выбранный класс: {current_class + 1}")
